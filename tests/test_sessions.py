...
def test_database_session_lifecycle(user: Any) -> None:
    ...
    payloads = [{"role": "user", "content": "hello"}, {"role": "assistant", "content": "hi"}]
...
def test_database_session_serializes_items(user: Any) -> None:
    ...
    assert stored.payload == {"type": "message", "payload": {"name": "hello", "count": 2}}
    assert session._get_items(limit=None) == [stored.payload]


@pytest.mark.django_db()
def test_get_session_uses_backend(user: Any) -> None:
    with override_settings(AGENTIC_DJANGO_SESSION_BACKEND="agentic_django.sessions.DatabaseSession"):
